...
router.get('/user-only-route', authMiddleware.authenticateUser, authMiddleware.authorizeUser, () => {
    // Route logic for the user-only route
});

// Apply the isAdmin middleware to restrict access to administrators
router.get('/admin-only-route', authMiddleware.authenticateUser, authMiddleware.isAdmin, () => {

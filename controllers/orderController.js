...
exports.createOrder = async (req, res) => {
    try {
        let k = req.body;
        const orders = await Orders.create(k.CustomerID, k.OrderDate, k.TotalAmount);
        res.status(200).send(orders);
    } catch (error) {
...
exports.updateOrder = async (req, res) => {
    try {
        let OrderId = req.params.OrderId;
        let k = req.body;
        const orders = await Orders.update(OrderId, k.CustomerID, k.OrderDate, k.TotalAmount);
        res.status(200).send(orders);
    } catch (err) {
        console.log('error', err);
        res.status(500).send({ message: 'server error' });
